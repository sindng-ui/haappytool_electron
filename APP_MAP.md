...
- **Interactions**:
  ...
  - **즐겨찾기 언더라인**: `Space` 키로 북마크를 지정하면, 텍스트 하단에 **옅은 황금색 언더라인**이 표시됩니다. 가장 미니멀하게 텍스트 가독성을 유지하면서 즐겨찾기 지점을 식별하는 디자인입니다. (액센트 바 로직은 주석 처리되어 있습니다.) [NEW]
...
- **Features**: 4가지 특화 분석, 하이퍼 로그 힌트 추출, **분석 단계별 Action 요약 표시 (가독성 개선)**. [UPDATED]
- **Core Interface**:
