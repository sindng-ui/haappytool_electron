...
- **Interactions & Shortcuts**:
  ...
  - **최신 최적화 (Extreme Performance)**: 
    - 필터링 시 동시 RPC 요청을 4개로 제한하여 IPC 부하 및 메모리 스파이크 방지. [THROTTLED]
    - 서브 워커에서 대용량 문자열 split 대신 Uint8Array 직접 순회 방식으로 변경하여 OOM 완전 차단. [ZERO-COPY]
    - 최대 지원 라인수를 20M으로 상향하여 초고밀도 로그 파일 대응. [SCALABLE]
    - 필터 변경 시 자동으로 1페이지로 이동 및 캐시 클리어 적용! [FIX] 🐧🚀
  - **수정 (Export Policy)**: Confluence 테이블 복사 시 `[` 문자 앞에 백슬래시(`\`)를 붙이는 대신, **제로 너비 공백(\u200B)**을 삽입하여 시각적 오염(￦) 없이 테이블 파싱 성공률을 높였습니다. 또한 `|` 문자는 전각 파이프(`｜`)로 대체하여 데이터 손실 없이 테이블 구조를 보호합니다! [MOD] 🐧💎
