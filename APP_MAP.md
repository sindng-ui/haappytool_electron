...
- **Interactions & Shortcuts**:
  ...
  - **수정 (Export Policy)**: Confluence 테이블 복사 시, 기존 `||` (Jira/Confluence 레거시 표기법) 대신 **표준 Markdown 형식(`|---|---|`)**을 적용하여 최신 Confluence 에디터에서 완벽하게 표(Table)로 인식하도록 수정했습니다. 또한, `[` 문자 앞에 **제로 너비 공백(\u200B)**을 삽입하여 시각적 오염(￦) 없이 테이블 파싱 성공률을 높였으며, `|` 문자는 전각 파이프(`｜`)로 대체하여 데이터 손실 없이 테이블 구조를 보호합니다! [MOD] 🐧💎
