...
- **Optimizations**:
  ...
  - `독립형 전체 찾기 (Find in All Open Files, Ctrl+Shift+F)`: Global Mission과 완전 독립된 룰로 열린 모든 파일을 즉시 검색. `FindInAllModal`(3섹션: 히스토리/해피콤보/블럭리스트), `FindInAllResultPanel`(우측패인 아래 Notepad++ 스타일 결과창, 드래그 높이 조절 & localStorage 저장, 닫기/재검색 버튼), `useFindInAllHistory`(최근 10개 Dexie DB 영구 저장, 호버 미리보기 툴팁, 클릭 불러오기), `useFindInAll`(검색 결과 Static 스냅샷, LogSession 리렌더 체인과 격리). Ctrl+Shift+F 단축키. 싱글뷰는 좌측패인 아래, 듀얼뷰는 우측패인 아래 표시. [NEW][HOT]
