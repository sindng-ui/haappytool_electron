# BigBrain Headless CLI User Guide 🐧🚀

형님, 이 가이드는 BigBrain의 백그라운드 구동 모드인 **Headless CLI (Command Line Interface)** 엔진의 사용법과 예제를 다룹니다! GUI를 열지 않고도 대규모 로그 파일을 필터링하거나, 백그라운드에서 복잡한 시스템 자동화 테스트(BlockTest)를 돌리고 싶을 때 활용하실 수 있는 마법의 지팡이입니다. 💎

---

BigBrain의 CLI 기반 모듈은 실행 환경에 따라 아래와 같은 방법으로 진입할 수 있습니다.

### 📦 패키징 배포판 세팅 (Production)
`BigBrain.exe` 파일이 있는 위치(보통 설치 폴더)에서 터미널을 열고 실행합니다.
```powershell
# 기본 사용법
.\BigBrain.exe cli [command] [options]

# 도움말 확인
.\BigBrain.exe cli --help
```
...
- **옵션:**
  - `-f, --filter <name>`: [필수] BigBrain GUI에 저장된 미션(필터 그룹) 이름
...
- **사용 예시:**
  ```bash
  # EXE 사용 시
  .\BigBrain.exe cli log-extractor -f "Crash Hunt" -i "C:\logs\syslog.txt"
  ```
...
### 6. `nettraffic` (네트워크 트래픽 자동 분석기) 📡🌐
네트워크 로그를 분석하여 엔드포인트 통계, User Agent 분포, 시간별 트래픽 인사이트를 추출합니다.

- **옵션:**
  - `-i, --input <path>`: **[Single Mode]** 분석할 단일 로그 파일 경로
  - `-l, --left <path>`: **[Compare Mode]** 기준이 되는 첫 번째(Base) 로그 파일
  - `-r, --right <path>`: **[Compare Mode]** 비교 대상이 되는 두 번째(Target) 로그 파일
  - `-o, --output <path>`: 결과 JSON 분석 리포트 저장 위치 (생략 시 `nettraffic_result_시간.json` 생성)

- **사용 예시:**
  ```bash
  # [Single] 단일 파일 분석 및 결과 JSON 저장
  .\BigBrain.exe cli nettraffic -i "./traffic.log" -o "./analysis.json"
  
  # [Compare] 두 로그 파일의 트래픽 증감 비교 (Diff)
  .\BigBrain.exe cli nettraffic -l "./old.log" -r "./new.log" -o "./diff.json"
  ```
...
저희 BigBrain CLI 모드는 평범한 터미널 스크립트가 아닙니다!
