...
네트워크 로그를 분석하여 엔드포인트 통계, User Agent 분포, 시간별 트래픽 인사이트를 추출합니다. 단일 파일 분석뿐만 아니라 두 로그 사이의 호스트별/UA별 증감 비교도 지원합니다.

- **옵션:**
  - `-i, --input <path>`: [단일 모드] 분석할 원본 로그 파일 경로
  - `-l, --left <path>`: [비교 모드] 기준이 되는 첫 번째 로그 파일
  - `-r, --right <path>`: [비교 모드] 비교 대상이 되는 두 번째 로그 파일
  - `-o, --output <path>`: 결과 JSON 분석 리포트 저장 위치 (생략 시 `nettraffic_result_시간.json` 생성)

- **사용 예시:**
  ```bash
  # 단일 파일 분석 및 결과 JSON 저장
  npm run cli -- nettraffic -i "./traffic.log" -o "./analysis.json"
  
  # 두 개 파일 트래픽 비교 분석 (Diff)
  npm run cli -- nettraffic -l "./old_traffic.log" -r "./new_traffic.log"
  ```
