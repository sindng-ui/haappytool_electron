...
- **Clipboard Fast Paste & View (2026-05-29)**: [NEW][HOT]
  - **Paste from Clipboard Button**: 파일이 없는 빈 탭(`LogViewerEmptyState.tsx`)에 Indigo Glow 스타일의 프리미엄 버튼 추가. 🐧📋⚡
  - **Ctrl+V Shortcut Detection**: 빈 탭이 활성화된 상태에서 `Ctrl+V` 단축키 감지 시 클립보드 데이터를 즉시 `[Clipboard] YYYY-MM-DD HH-mm.log` 가상 파일 객체로 변환하여 자동 로드. 기존 WASM 핵심 인덱서 및 필터링과 100% 호환. 🐧🚀⚡
