...
const PreviewCard = React.memo(({ releaseName, version, releaseDate, tags }: any) => (
    <div className="w-full bg-[#161e2e] border border-slate-700 rounded-2xl p-6 flex flex-col gap-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)] border-t-white/5">
        <div className="flex justify-between items-center">
            <span className="text-[11px] font-black text-slate-400 truncate max-w-[130px] tracking-widest uppercase">
                {releaseName || 'UNNAMED'}
            </span>
            <span className="text-[10px] font-black text-indigo-400 bg-indigo-500/15 px-3 py-1 rounded-lg border border-indigo-500/30">
                {releaseDate
        ...
        <div className="flex items-center gap-4 py-2">
            <div className="w-1.5 h-12 rounded-full bg-gradient-to-b from-indigo-500 via-purple-500 to-pink-500 shrink-0 shadow-[0_0_15px_rgba(99,102,241,0.5)]" />
            <span className="text-4xl font-black text-white tracking-tighter leading-none break-all drop-shadow-lg">
        ...
        <div className="flex flex-wrap gap-2.5 mt-1">
            {(tags.length > 0 ? tags : ['TAG']).map((t: string) => (
                <span key={t}
                    className="px-4 py-1.5 rounded-lg text-[11px] font-black text-white uppercase tracking-wider shadow-sm border border-white/5"
...
const AddReleaseModal: React.FC<AddReleaseModalProps> = ({
    ...
    return (
        <div
            style={{ position: 'fixed', inset: 0, top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100vh', zIndex: 9999, backgroundColor: '#020817', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px 16px' }}
        >
            {/* Modal Container: Flex row to hold both form and preview */}
            <div
                className="bg-[#0f172a] border-2 border-slate-800 rounded-[32px] shadow-2xl w-full flex overflow-hidden"
                style={{ height: 'calc(100vh - 40px)', maxWidth: 'min(1400px, calc(100vw - 32px))' }}
                lang="en"
            >
                {/* ══ LEFT: Form (Untouched) ══ */}
                <div className="flex-1 flex flex-col min-h-0 bg-[#0f172a]">
                    ...
                    {/* ── 3. Footer ── */}
                    <div className="shrink-0 px-6 py-4 border-t border-slate-800 flex justify-between items-center bg-[#161e2e]">
                        <button
                            onClick={onClose}
                ...
                <div className="w-[360px] shrink-0 flex flex-col border-l border-slate-800 bg-[#0a1120]">
                    <div className="shrink-0 px-10 py-6 border-b border-slate-800 bg-[#161e2e]">
                        <h3 className="text-sm font-black text-white tracking-tight uppercase">Preview</h3>
                        <p className="text-[10px] text-slate-500 font-black tracking-[0.3em] uppercase mt-1">Real-time Node Status</p>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-8 flex flex-col gap-8">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-5 bg-[#161e2e] rounded-2xl border border-slate-800/80 text-center shadow-lg hover:border-indigo-500/30 transition-all group">
                                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2 group-hover:text-indigo-400 transition-colors">Target Scope</p>
                                <p className="text-3xl font-black text-white tracking-tighter">{years.length}</p>
                            </div>
                            <div className="p-5 bg-[#161e2e] rounded-2xl border border-slate-800/80 text-center shadow-lg hover:border-indigo-500/30 transition-all group">
                                <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2 group-hover:text-indigo-400 transition-colors">Context Nodes</p>
                                <p className="text-3xl font-black text-white tracking-tighter">{tags.length}</p>
                            </div>
                        </div>

                        <div className="mt-auto pt-4">
                            <div className="p-4 bg-indigo-500/5 rounded-2xl border border-indigo-500/10 border-dashed text-center">
                                <p className="text-[10px] font-bold text-indigo-400/60 uppercase tracking-widest leading-relaxed">
                                    Preview reflects all changes<br/>currently in the editor
                                </p>
                            </div>
                        </div>
                    </div>
