...
const Sidebar: React.FC<SidebarProps> = ({ activePluginId, onSelectPlugin, pluginOrder, onReorderPlugins, onOpenSettings, plugins, enabledPlugins }) => {
  ...
  let activePlugins = pluginOrder
    ...
    .filter((p): p is BigBrainPlugin => !!p);
  ...
  return (
    <div className="h-full relative shrink-0 z-[100]">
      {/* Placeholder to reserve layout space */}
      <div className="w-14 h-full" />

      {/* Floating Animated Sidebar - 100% GPU Accelerated using clip-path */}
      <div
        className="absolute top-0 left-0 h-full bg-gradient-to-b from-slate-950 via-[#0B0F19] to-slate-950 border-r border-indigo-500/20 text-slate-400 transform-gpu backface-hidden outline-none z-[100] flex flex-col will-change-[clip-path]"
        style={{
          width: '224px',
          contain: 'strict',
          clipPath: isExpanded ? 'inset(0 0 0 0)' : 'inset(0 168px 0 0)',
          transition: 'clip-path 700ms cubic-bezier(0.19,1,0.22,1), background 700ms'
        }}
        tabIndex={0}
        onBlur={handleBlur}
      ...
      >
        <div className="w-[224px] h-full flex flex-col shrink-0 relative">
          ...
          <div className="flex-1 px-1 py-2 space-y-2 overflow-y-auto overflow-x-hidden custom-scrollbar shrink-0">
            {activePlugins.map((plugin) => {
              ...
              return (
                ...
                >
                  <div className={`absolute left-1 top-1/2 -translate-y-1/2 cursor-grab active:cursor-grabbing transition-opacity duration-300 ${isExpanded ? 'opacity-0 group-hover:opacity-30' : 'opacity-0 pointer-events-none'}`}>
                  ...
                  <span className={`ml-1 font-medium text-[15px] whitespace-nowrap transition-all duration-300 ${isExpanded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'
                    }`}>
                    {plugin.name}
                  </span>
            ...
            {labPlugins.length > 0 && (
              <div className="mt-4 pt-4 border-t border-white/5">
                ...
                >
                  ...
                  <span className={`ml-1 font-medium text-[13px] uppercase tracking-wider whitespace-nowrap transition-all duration-300 flex-1 text-left ${isExpanded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>
                    Lab
                  </span>
          ...
          <div className="px-1 pb-4 w-[224px] mt-auto space-y-3 shrink-0">
          ...
          <div className={`p-4 transition-opacity duration-300 shrink-0 ${isExpanded ? 'opacity-100' : 'opacity-0'}`}>
