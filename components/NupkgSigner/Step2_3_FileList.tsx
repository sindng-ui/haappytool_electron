...
const Step2_3_FileList: React.FC<Props> = ({ soFiles, onToggleChecked, onSignedUpload, onProcess, onBack }) => {
    ...
    useEffect(() => {
        localStorage.setItem('nupkg_isms_url', ismsUrl);
    }, [ismsUrl]);

    const handleOpenIsms = () => {
        let url = ismsUrl.trim();
        if (!url) return;
    ...
    return (
        <div className="flex flex-col h-full max-w-5xl mx-auto w-full">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-8 py-2.5 rounded-2xl font-bold transition-all ${checkedCount > 0
            ...
            {/* ISMS URL Tool Bar */}
            <div className="mb-6 flex items-center gap-4 p-4 bg-white/50 dark:bg-slate-900/50 backdrop-blur-md rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all hover:shadow-md">
                <div className="flex items-center gap-3 px-4 py-2 bg-indigo-50 dark:bg-indigo-500/10 rounded-2xl text-indigo-600 dark:text-indigo-400">
                    <Globe size={18} />
                    <span className="text-xs font-black uppercase tracking-widest">ISMS URL</span>
                </div>
                ...
                <div className="flex-1 relative group">
                    <input
                        type="text"
                        value={ismsUrl}
                        onChange={(e) => setIsmsUrl(e.target.value)}
                        placeholder="https://isms.sec.samsung.net/..."
                        className="w-full bg-slate-100 dark:bg-slate-800 border-2 border-transparent focus:border-indigo-500/50 focus:bg-white dark:focus:bg-slate-950 px-5 py-2.5 rounded-2xl text-sm font-bold transition-all outline-none text-slate-700 dark:text-slate-200"
                    />
                ...
                {soFiles.length === 0 ? (
                    ...
                    soFiles.map((file) => (
                        <div
                            ...
                            className={`grid grid-cols-[auto_1fr_auto_240px_auto] gap-4 px-8 py-5 border-b border-slate-100 dark:border-slate-900 items-center transition-all duration-300 relative group ${!file.checked
                        ...
                        >
                            ...
                            <button
                                ...
                                className={`w-8 h-8 rounded-xl border-2 transition-all flex items-center justify-center ${file.checked
                            ...
                            <div className="w-24 flex justify-center">
                                <button
                                    ...
                                    className={`p-2.5 rounded-xl transition-all ${file.checked
                            ...
                            <div className="w-[240px] flex items-center justify-center h-full">
                                {file.checked ? (
                                    <div className="relative w-full h-14 group/drop">
                                        {file.signedBlob ? (
                                            <div className={`flex items-center gap-3 px-4 h-full rounded-2xl border-2 transition-all overflow-hidden relative ${file.isSigned
                                            ...
                                            <label className={`w-full h-full flex items-center justify-center gap-3 rounded-2xl border-2 border-dashed transition-all cursor-pointer ${dragOverPath === file.path
