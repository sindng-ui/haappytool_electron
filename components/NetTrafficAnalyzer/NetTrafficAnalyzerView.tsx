...
const NetTrafficAnalyzerView: React.FC = () => {
      ...
      >
        {file ? (
          <div className="flex flex-col items-center">
            ...
            <button className="text-[9px] text-rose-400 mt-1 hover:underline" onClick={() => {
                        ...
                        <div className="flex items-start space-x-1.5">
                          ...
                          <div className="flex flex-col min-w-0 flex-1">
                             ...
                             <div className="flex items-center mt-1 space-x-2 text-[9px] opacity-40">
                                <span>Alias: {group.alias || 'Unknown'}</span>
  ...
  const renderUATable = (data: UAResult[], title: string) => {
    return (
      <div className="flex flex-col h-full bg-[#0b0f19] rounded-lg border border-rose-500/20 overflow-hidden">
        <div className="bg-[#0f172a] h-8 px-3 flex items-center justify-between border-b border-rose-500/30">
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{title}</span>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          <table className="w-full text-left border-collapse text-[11px]">
            <thead className="sticky top-0 bg-[#0f172a] shadow-sm text-slate-500 font-bold uppercase tracking-tighter z-10">
              <tr>
                <th className="px-3 py-1.5 border-b border-rose-500/10">Identified Context Details</th>
                <th className="px-3 py-1.5 border-b border-rose-500/10 text-right w-20">Hits</th>
              </tr>
            </thead>
            <tbody>
              {data.map((row, idx) => {
                ...
                return (
                  <React.Fragment key={idx}>
                    <tr 
                      className={`hover:bg-rose-500/5 border-b border-slate-900 transition-colors cursor-pointer ${isExpanded ? 'bg-rose-500/10' : ''}`}
                      onClick={() => toggleExpand(uaKey)}
                    >
                      <td className="px-2 py-2">
                        <div className="flex items-start space-x-1.5">
                          <Lucide.ChevronRight size={14} className={`mt-0.5 transition-transform ${isExpanded ? 'rotate-90 text-rose-400' : 'text-slate-600'}`} />
                      ...
                      <td className="px-3 py-2 text-right font-black text-rose-400 font-mono text-xs">
                    ...
                    {isExpanded && (
                      <tr className="bg-slate-900/30">
                         <td colSpan={2} className="p-0">
                            <div className="py-2 px-3 pl-8 space-y-2 border-l-2 border-rose-500/20 ml-4">
                                     ...
                                     <tbody>
                                        {row.endpoints.map((ep, eIdx) => (
                                          <tr key={eIdx} className="border-b border-slate-900 hover:bg-slate-900/50">
                                            <td className="px-2 py-1 font-mono text-rose-300 opacity-80">{ep.templateUri}</td>
                                            <td className="px-2 py-1 text-right font-bold text-rose-400">{ep.totalCount}</td>
                                          </tr>
                                        ))}
                                     </tbody>
                                  </table>
              ...
              <div className="space-y-1">
                {Object.entries(insights.hosts).sort((a: any, b: any) => (b[1] as number) - (a[1] as number)).slice(0, 5).map(([host, count]) => (
                  <div key={host} className="flex justify-between items-center text-[9px]">
                    <span className="truncate text-slate-400 max-w-[80px] font-mono">{host}</span>
                    <span className="font-bold text-slate-300 font-mono">{count as number}</span>
                  </div>
                ))}
              </div>
           </div>
