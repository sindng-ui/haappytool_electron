...
네비게이션**: 상단 고정바에 PREV/NEXT 버튼을 추가하여 분석 세그먼트를 순차적으로 탐색할- [x] Timeline 네비게이션 및 UI 정밀 개선
    - [x] 구현 계획서(docs/implementation_plan_timeline_v2.md) 작성 및 승인
    - [x] 네비게이션 이동 시 리스트 내 자동 스크롤(Focus) 로직 구현
    - [x] FROM-TO 연결선(Connector) UI 리뉴얼 (SVG/CSS 일체형 디자인)
